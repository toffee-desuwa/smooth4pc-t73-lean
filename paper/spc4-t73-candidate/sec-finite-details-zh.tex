...
$(-2)^3(-328)=2624$。非零结论不变。
将 $u$ 与 $\ell$ 同置于 THXY 坐标而词仍在领坐标中，得到 $-2496$；由于算子未被传输，这不是整个配对的坐标变换，也不是第二个几何类（附录~\ref{app:legality}）。端点传输程序将这些被撤回的数值重现为非法的部分传输：THXY cup 配领圈 cap 得 $-59072$；THXY cup 与 cap 配未传输的词得 $-2496$；冻结切割槽位 cup 配领圈 cap 得 $-55104$；槽位 cup 与 cap 配未传输的词得 $-115456$。词、cup 与 cap 的同步传输在每种坐标系下都给出 $2624$，且推导出的常数项恰为 \eqref{eq:u} 与 \eqref{eq:ell}。值 $2624$ 属于
