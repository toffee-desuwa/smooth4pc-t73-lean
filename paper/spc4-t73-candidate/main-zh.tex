...
已重放的有限证书确立：(i) $A$ 的 93 个 transvection 分解与具 GAP 自由基的 93 位侧模式；(ii) 含 42 个 $y$ 通道的精确紧凑 $m_2$ 词与 $\operatorname{lk}(m_2,r_{yz})=0$（引理~\ref{lem:P0d-link}）；(iii) 等于公开 $11340$ 字母词的六扫掠词；(iv) 引理~\ref{lem:P0a} 的 handlebody 桥梁：Johnson 脊与 AR 脊的对偶块正则邻域及显式坍缩序列，由 $S$ 互相运输。假设~\ref{hyp:P0} 的下列义务为 \Open：(a) 将 mapping-torus handlebody 与该对等同，这是 Aitchison--Rubinstein 构造所需而证书未使用的；(b) 由 93 个 Johnson 生成元复合的实际 PL 同胚 $\psi_A$ 与实际 Aitchison--Rubinstein 带框 link \eqref{eq:ARcore}--\eqref{eq:ARannulus}；(c) 带 belt 球面、band slide 与法向场运输的 $(t,h_{CS})$ 与 $(x,m_1)$ 实际 Kirby 消去（引理~\ref{lem:P0b}）；(d) 从实际消去后 link 切出的领圈股及其与 MWW 缆 framing 的同一性（引理~\ref{lem:P0c}）；(e) 闭图景与 $\Sigma_A^0$ 的识别。故假设~\ref{hyp:P0} 为 \Open。
\end{proposition}

\begin{proof}
(i)--(iv) 是附录~\ref{sec:reproducibility-extended} 所列可重放证书；(iv) 即引理~\ref{lem:P0a} 的证明。(a)--(e) 未由其确立：已提交的领圈股由公开词经控制构造生成，消去 movie 是 $\mathbb R^3$ 中的局部模型而非实际附着 link 上的移动。
\end{proof}
