...
输入 & 状态 & 已确立 / 待完成 \\
\midrule
P0a（handlebody 桥梁） & \Discharged & 重心重分中四个脊的对偶块正则邻域、到脊的显式初等坍缩序列、亏格三公共边界、并集为 $T^3$、$S$ 对完整对的运输，以及 Regina handlebody 识别（引理~\ref{lem:P0a}）。 \\
P0b（两个带框消去） & \Open & 93 个 Johnson 单位移动的显式 PL 同胚、诱导映射为 $A$ 的复合及截面拉直已认证；该方形推动代表元不保持 handlebody 对且其复合脊像几何增长，故实际 Aitchison--Rubinstein 带框 link 及其带 belt 球面、band slide 与法向场运输的实际 Kirby 消去均未构造。 \\
...
\begin{proof}
将 Johnson 周期一环面放大 $4$ 倍、AR 周期二环面放大 $2$ 倍，二者都成为周期四的 Freudenthal 网格 $T$（$64$ 个顶点、$384$ 个四面体），其上 $S(u)=2u-(1/2,1/2,1/2)$ 即平移 $T(v)=v-(1,1,1)$，是 $T$ 的单纯自同构。Johnson 脊 $K_J^0,K_J^1$ 为过 $(0,0,0)$ 与 $(2,2,2)$ 的坐标轴圆，AR 脊 $K_{AR}^0,K_{AR}^1$ 为过 $(-1,-1,-1)$ 与 $(1,1,1)$ 者；每个都是三圆玫瑰，$10$ 个顶点、$12$ 条边（秩 $3$）。在第一重心重分 $T'$（$1664$ 个顶点、$9216$ 个四面体）中，令 $H^0=N(K^0;T')=\bigcup_{\sigma\in K^0}D(\sigma)$ 为 $K^0$ 的对偶块邻域，$H^1$ 为其补的闭包。仓库证书对 Johnson 对与 AR 对分别给出显式初等坍缩序列 $H^0\searrow K^0$（$1440$ 个四面体上 $3944$ 步自由面坍缩）与 $H^1\searrow K^1$（$7776$ 个四面体上 $17688$ 步），均由独立检查器逐步重放；按 Rourke--Sanderson，坍缩到秩三图的紧 $3$-流形是其正则邻域，故为亏格三 handlebody。每对的两个 handlebody 不相交、并集为整个 $T'$，且共享一个含 $1056$ 个三角形、Euler 特征 $-4$ 的闭连通边界曲面。平移 $T$ 将 $H_J^0$ 逐四面体映到 $H_{AR}^0$、$H_J^1$ 映到 $H_{AR}^1$，其他平移则失败。Regina~7.4 独立报告四个复形均为亏格三 handlebody（\texttt{recogniseHandlebody()}$=3$；在简化后的三角剖分上 $H_1=\mathbb Z^3$，每个七个四面体）。半径 $1/196104$ 关于 $0$ 的保护球映到关于 $Q$ 半径 $2/196104$ 的球。未使用正则邻域唯一性。重放：\texttt{scripts/verify\_elementary\_collapse.py --check} 与 \texttt{scripts/verify\_t73\_handlebody\_bridge\_regina.py --check}。
\end{proof}
...
\Open。已确立。(1) 对 93 个单位移动中的每一个，仓库记录了 $T^3$ 的显式 PL 同胚 $\psi_k=\Pi_k\circ E_k$：$E_k$ 为线性 transvection，$\Pi_k$ 为方形推动，它把厚板 $Q_k\times[-1/8,1/8]$ 的 24 四面体棱柱胞分解中的一个顶点移到侧位所指定的弯折位置；每个胞带有仿射映射与逆，每个像胞严格正定向，支撑与三条坐标轴圆及顶点的 sup 距离至少 $1/8$，且在 $H_1$ 上诱导的映射复合为 $A$（\texttt{geometry/t73\_johnson\_generators/}、\texttt{scripts/verify\_t73\_pl\_homeomorphism.py}）。(2) 复合 $\psi_A=\psi_{93}\circ\cdots\circ\psi_1$ 在关于顶点半径 $1/196104$ 的立方体上是线性映射 $A$，因为推动因子固定该立方体而线性因子不固定；显式截面拉直 $\theta$（186 层嵌套立方壳、$13{,}416$ 个胞、全部正定向）使 $\Psi=\psi_A\circ\theta$ 在半径 $1/(196104\cdot3^{186})$ 的立方体上为恒等，而单壳径向拉直对本 $A$ 不可能，因为 $A_{11}=0$ 迫使某胞行列式在任何半径下符号为负。(3) 在自由群词层面 $\psi_A(x)=z$，故第一次消去后 $m_1=zx^{-1}$；证书含两个 $\mathbb R^3$ 中几何交数为 $1$ 的标准局部消去 movie。
未确立，且阻碍已明确。(a) 该代表元不保持 handlebody 对：每个生成元的弯折脊像都以 sup 距离 $1/4$ 离开引理~\ref{lem:P0a} 的对偶块 handlebody $H_J^0$（见证 $(0,0,1/4)\mapsto(0,1/4,1/4)\notin H_J^0$），故 Aitchison--Rubinstein 在 Heegaard 曲面上的附着曲线 $\psi_A(C_i^+)$ 对它没有定义；所需的是精确保持该对的代表元，即支撑在 prefix 管边界附近的 slide 映射，尚未构造。(b) 脊圈在 93 重复合下的精确像不是可存储的多边形：顶点数按每个生成元约 $1.21$--$1.25$ 倍几何增长（$C_2$ 经 55 个生成元后有 $366{,}822$ 个顶点，外推到 93 约 $5\cdot10^8$），而同调类受部分积的元素界定，故实际附着 link 不能通过把脊圈推过该复合得到。(c) 因此实际带框附着 link \eqref{eq:ARcore}--\eqref{eq:ARannulus} 及其上带三角化 belt 球面、band slide、分量运输与法向场运输的 $(t,h_{CS})$ 与 $(x,m_1)$ 消去仍未构造。词层面恒等式不能替代这些嵌入对象。
\end{remark}
...
已重放的有限证书确立：(i) $A$ 的 93 个 transvection 分解与具 GAP 自由基的 93 位侧模式；(ii) 含 42 个 $y$ 通道的精确紧凑 $m_2$ 词与 $\operatorname{lk}(m_2,r_{yz})=0$（引理~\ref{lem:P0d-link}）；(iii) 等于公开 $11340$ 字母词的六扫掠词；(iv) 引理~\ref{lem:P0a} 的 handlebody 桥梁：Johnson 脊与 AR 脊的对偶块正则邻域及显式坍缩序列，由 $S$ 互相运输；(v) 93 个 Johnson 单位移动的显式 PL 同胚、诱导映射为 $A$ 的复合，以及引理~\ref{lem:P0b} 的截面拉直 $\theta$。假设~\ref{hyp:P0} 的下列义务为 \Open：(a) 将 mapping-torus handlebody 与该对等同，这是 Aitchison--Rubinstein 构造所需而证书未使用的；(b) 精确保持该对的 $\psi_A$ 代表元（(v) 的方形推动代表元每一步都把脊推出 $H_J^0$）及由它构造的实际 Aitchison--Rubinstein 带框 link \eqref{eq:ARcore}--\eqref{eq:ARannulus}，其脊像不能通过把多边形推过该复合得到（顶点几何增长，引理~\ref{lem:P0b}）；(c) 带 belt 球面、band slide 与法向场运输的 $(t,h_{CS})$ 与 $(x,m_1)$ 实际 Kirby 消去（引理~\ref{lem:P0b}）；(d) 从实际消去后 link 切出的领圈股及其与 MWW 缆 framing 的同一性（引理~\ref{lem:P0c}）；(e) 闭图景与 $\Sigma_A^0$ 的识别。故假设~\ref{hyp:P0} 为 \Open。
\end{proposition}

\begin{proof}
(i)--(v) 是附录~\ref{sec:reproducibility-extended} 所列可重放证书；(iv) 即引理~\ref{lem:P0a} 的证明，(v) 是引理~\ref{lem:P0b} 已确立的部分。(a)--(e) 未由其确立：已提交的领圈股由公开词经控制构造生成，消去 movie 是 $\mathbb R^3$ 中的局部模型而非实际附着 link 上的移动。
\end{proof}
...
\tag{27}
\]
在 $s_0$ 之下第二个等式为定义；Frobenius 恒等式与不相交支撑交换证明第一个等式及所有 owner 方块可交换。该逐态构造的代数在 \texttt{Smooth4PC/ReynoldsCableCocone.lean} 中形式化：对每个态 $r\in\mathbb N^n$，Reynolds 平均在置换放置的 braid 作用下不变，两个添加对关系由带常数纤维的逐放置关系推出，$D_r$ 沿迭代一点添加向 $s_0$ 以下各态的延拓与公共上界无关且在每个态满足 $\psi^{[1]}$ 关系。该文件的每个几何输入（实际 MWW beta 作用置换放置、实际添加对遗忘一个 distinguished 对且纤维常数并可交换、$s_0$ 以上的下降）在其中都是假设；对实际缆化源，这些假设属于 \Open 的假设~\ref{hyp:P1}。
